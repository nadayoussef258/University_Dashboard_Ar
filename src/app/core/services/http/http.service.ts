    ...
    if (event.statusCode) {
      ...
      switch (status) {
        case NumberedHttpStatus.BadRequest: {
          if (event.errors && Array.isArray(event.errors) && event.errors.length) {
            const errorMessages = event.errors.map((error) => this.localize.translate.instant('VALIDATION.' + event.message)).join(', ');

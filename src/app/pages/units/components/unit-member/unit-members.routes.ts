...
export const unitMembersRoutes: Routes = [
  {
    path: '',
    loadComponent: () => import('./unit-members.component').then((c) => c.UnitMembersComponent),
    data: { pageTitle: 'PAGES.UNIT_MEMBERS.MAIN.PAGE_TITLE', pageType: 'list' },
    canActivate: [clearUnitIdGuard]
  },
  {
    path: 'add',
    loadComponent: () => import('../add-edit-unit-member/add-edit-unit-member.component').then((c) => c.AddEditUnitMemberComponent),

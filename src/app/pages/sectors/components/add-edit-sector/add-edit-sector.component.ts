...
@Component({
  ...
  imports: [CardModule, FormsModule, TabsModule, ReactiveFormsModule, PrimeAutoCompleteComponent, PrimeInputTextComponent, SubmitButtonsComponent, TranslatePipe],
  templateUrl: './add-edit-sector.component.html',
  styleUrl: './add-edit-sector.component.css'
})
//
export class AddEditSectorComponent extends BaseEditComponent implements OnInit {
  sectorId: string = '';
  ...
  fetchPagesDetails(sector: any) {
    this.pagesService.pages.subscribe((response: any) => {
      ...
      this.selectedPage = this.filteredPages.find((page: any) => page.id === sector.pageId);
  ...
  fetchAboutDetails(sector: any) {
    this.aboutService.abouts.subscribe((response: any) => {
      ...
      this.selectedAbout = this.filteredAbout.find((about: any) => about.id === sector.aboutId);
      this.form.get('aboutId')?.setValue(this.selectedAbout.id);
    });
  }
  ...
  submit() {
    ...
    if (this.pageType === 'edit')
      this.sectorsService.update({ id: this.id(), ...this.form.value }).subscribe(() => {
        this.redirect();
      });
  }

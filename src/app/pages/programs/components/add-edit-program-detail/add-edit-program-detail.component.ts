...
import { PrimeAutoCompleteComponent, PrimeInputTextComponent, ProgramDetailsService, SubmitButtonsComponent, ProgramsService } from '../../../../shared';
...
//
export class AddEditProgramDetailComponent extends BaseEditComponent implements OnInit {
  programId: string = '';
  selectedProgram: any;
  ...
  fetchProgramDetails(programDetail: any) {
    this.programsService.programs.subscribe((response: any) => {
      ...
      this.selectedProgram = this.filteredPrograms.find((program: any) => program.id === programDetail.id);
      this.form.get('programId')?.setValue(this.selectedProgram.id);
    });
  }

  getEditProgramDetail = () => {
    this.programDetailsService.getEditProgramDetail(this.id()).subscribe((programDetail: any) => {
      this.initFormGroup();
  ...
  submit() {
    ...
    if (this.pageType === 'edit')
      this.programDetailsService.update({ id: this.id(), ...this.form.value }).subscribe(() => {
        this.redirect();
      });
  }
